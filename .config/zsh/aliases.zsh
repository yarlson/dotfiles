...
__ide_tmux_layout() {
	local dir cols lines created session left_pane right_pane right_width
	...
	created="$(tmux new-session -d -x "$cols" -y "$lines" -P -F '#{session_id} #{pane_id}' -c "$dir" 'nvim .')" || return
	session="${created%% *}"
	left_pane="${created##* }"
